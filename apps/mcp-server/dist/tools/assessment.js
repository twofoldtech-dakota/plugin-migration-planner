import { z } from "zod";
import { getDb, saveAssessment as dbSaveAssessment, getAssessmentById, getAssessmentByPath, listAssessmentsByPath, setActiveAssessment as dbSetActiveAssessment, getActiveAssessmentId, getClientById, } from "@migration-planner/db";
...
export async function getAssessment(input) {
    const db = getDb();
    let row = null;
    if (input.id) {
...
export async function listAssessmentsForPath(input) {
    const db = getDb();
...
export async function setActiveAssessment(input) {
    const db = getDb();
