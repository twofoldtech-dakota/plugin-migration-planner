import { t as eq, j as assessments, D as like, x as and, b as desc, u as discoveryDimensions, s as sql, a as assumptions, e as estimateSnapshots, E as gt, F as gte, G as lte, H as asc } from './db-BWpbog7L.js';

async function queryProjects(db, input) {
  const conditions = [];
  if (input.status) {
  ...
  }).from(assessments).where(conditions.length > 0 ? and(...conditions) : void 0).orderBy(desc(assessments.created_at)).limit(input.limit ?? 50);
  const projects = await Promise.all(rows.map(async (row) => {
    const dpRows = await db.select({
    ...
    }).from(discoveryDimensions).where(eq(discoveryDimensions.assessment_id, row.id));
    const dp = dpRows[0];
    const completeness_pct = dp && dp.total_dimensions > 0 ? Math.round(((dp.complete ?? 0) + (dp.partial ?? 0) * 0.5) / dp.total_dimensions * 1e3) / 10 : null;
    const asmpRows = await db.select({
      total_assumptions: sql`count(*)`,
      validated: sql`sum(case when ${assumptions.validation_status} = 'validated' then 1 else 0 end)`
    }).from(assumptions).where(eq(assumptions.assessment_id, row.id));
    const asmp = asmpRows[0];
    const estRows = await db.select({
    ...
    }).from(estimateSnapshots).where(eq(estimateSnapshots.assessment_id, row.id)).orderBy(desc(estimateSnapshots.version)).limit(1);
    const est = estRows[0];
    return {
      ...row,
      completeness_pct,
      ...
      recommended_hours: est?.client_summary && typeof est.client_summary === "object" && est.client_summary !== null ? est.client_summary.recommended_hours ?? null : null
    };
  }));
  return { projects, total: projects.length };
}
async function queryConfidenceTimeline(db, input = {}) {
  ...
  }).from(estimateSnapshots).innerJoin(assessments, eq(estimateSnapshots.assessment_id, assessments.id)).where(and(...conditions)).orderBy(asc(estimateSnapshots.created_at)).limit(input.limit ?? 500);
  return rows;
}

export { queryConfidenceTimeline as a, queryProjects as q };
//# sourceMappingURL=analytics-BZLNJxd8.js.map
