import { a as assumptions, x as and, t as eq, s as sql, v as discoveryAnswers, j as assessments } from './db-BWpbog7L.js';

async function updateAssumption(db, input) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const existing = await db.select().from(assumptions).where(and(eq(assumptions.id, input.assumption_id), eq(assumptions.assessment_id, input.assessment_id))).limit(1);
  ...
  }).from(assumptions).where(eq(assumptions.assessment_id, input.assessment_id));
  const confirmedRows = await db.select({
    c: sql`count(*)`
  }).from(discoveryAnswers).where(and(eq(discoveryAnswers.assessment_id, input.assessment_id), eq(discoveryAnswers.confidence, "confirmed")));
  const totalAnswerRows = await db.select({
    c: sql`count(*)`
  }).from(discoveryAnswers).where(eq(discoveryAnswers.assessment_id, input.assessment_id));
...
export { updateAssumption as u };
//# sourceMappingURL=assumptions-Czeu5zeL.js.map
