...
import { estimateSnapshots, estimateComponents, assessments } from "../schema.js";
...
export async function listEstimateVersions(
  ...
  const rows = await db
    ...
    .where(eq(estimateSnapshots.assessment_id, assessmentId))
...
export async function saveEstimate(db: Database, input: SaveEstimateInput) {
  const now = new Date().toISOString();

  // Determine next version
  const maxVersionRows = await db
    .select({ v: max(estimateSnapshots.version) })
    .from(estimateSnapshots)
    .where(eq(estimateSnapshots.assessment_id, input.assessment_id));
...
export async function getEstimate(
  ...
  if (version) {
    const rows = await db
      ...
      .where(
        and(
          eq(estimateSnapshots.assessment_id, assessmentId),
          eq(estimateSnapshots.version, version)
        )
      )
      .limit(1);
    snapshot = rows[0];
  } else {
    // Latest version via subquery
    const rows = await db
      .select()
      .from(estimateSnapshots)
      .where(eq(estimateSnapshots.assessment_id, assessmentId))
      .orderBy(sql`${estimateSnapshots.version} DESC`)
      .limit(1);
    snapshot = rows[0];
  }
  ...
  const components = await db
    ...
    .where(eq(estimateComponents.snapshot_id, snapshot.id));

  // Reconstruct phases from components
  const phaseMap: Record<string, { id: string; name: string; components: unknown[] }> = {};
  ...
  // Use stored phases_json order if available
  let phases: unknown[];
  const storedPhases = snapshot.phases_json as Array<Record<string, unknown>> | null;
  if (storedPhases && Array.isArray(storedPhases) && storedPhases.length > 0) {
    phases = storedPhases.map((sp) => ({
      id: sp.id,
      name: sp.name,
      duration: sp.duration,
      components: phaseMap[sp.id as string]?.components ?? [],
    }));
  } else {
