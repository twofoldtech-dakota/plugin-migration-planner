...
export async function updateAssumption(db: Database, input: UpdateAssumptionInput) {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(assumptions.id, input.assumption_id),
        eq(assumptions.assessment_id, input.assessment_id)
      )
    )
    .limit(1);
  ...
  const summaryRows = await db
    .select({
      ...
      total_widening: sql<number>`sum(case when ${assumptions.validation_status} = 'unvalidated' then ${assumptions.pessimistic_widening_hours} else 0 end)`,
    })
    .from(assumptions)
  ...
  const confirmedRows = await db
    ...
    .where(
      and(
        ...
        eq(discoveryAnswers.confidence, "confirmed")
      )
    );
  ...
  const confidence =
    totalDataPoints > 0 ? Math.round(((validated + confirmed) / totalDataPoints) * 100) : 0;
