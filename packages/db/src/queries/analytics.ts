import { eq, sql, desc, like, and, max, gt, gte, lte, asc } from "drizzle-orm";
...
export async function queryProjects(db: Database, input: QueryProjectsInput) {
  ...
  const projects = await Promise.all(
    rows.map(async (row) => {
      ...
      const dpRows = await db
        .select({
          total_dimensions: sql<number>`count(*)`,
          complete: sql<number>`sum(case when ${discoveryDimensions.status} = 'complete' then 1 else 0 end)`,
          partial: sql<number>`sum(case when ${discoveryDimensions.status} = 'partial' then 1 else 0 end)`,
        })
      ...
      const completeness_pct =
        dp && dp.total_dimensions > 0
          ? Math.round(
              (((dp.complete ?? 0) + (dp.partial ?? 0) * 0.5) / dp.total_dimensions) * 1000
            ) / 10
          : null;
      ...
      const asmpRows = await db
        .select({
          total_assumptions: sql<number>`count(*)`,
          validated: sql<number>`sum(case when ${assumptions.validation_status} = 'validated' then 1 else 0 end)`,
        })
        .from(assumptions)
      ...
      return {
        ...
        recommended_hours:
          ...
          est.client_summary !== null
            ? (est.client_summary as Record<string, unknown>).recommended_hours ?? null
            : null,
      };
    })
  );
...
export async function queryConfidenceTimeline(
  db: Database,
  input: QueryConfidenceTimelineInput = {}
): Promise<ConfidenceTimelinePoint[]> {
