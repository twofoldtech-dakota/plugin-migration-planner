...
export async function saveAnalysis(db: Database, input: SaveAnalysisInput) {
...
export async function getAnalysis(db: Database, assessmentId: string) {
  ...
  const gapRows = await db
    .select({
      unknown_answers: sql<number>`sum(case when ${discoveryAnswers.confidence} = 'unknown' then 1 else 0 end)`,
      assumed_answers: sql<number>`sum(case when ${discoveryAnswers.confidence} = 'assumed' then 1 else 0 end)`,
      confirmed_answers: sql<number>`sum(case when ${discoveryAnswers.confidence} = 'confirmed' then 1 else 0 end)`,
      total_answers: sql<number>`count(*)`,
    })
    .from(discoveryAnswers)
    .where(eq(discoveryAnswers.assessment_id, assessmentId));

  if (riskRows.length === 0 && assumRows.length === 0) return null;

  // Aggregate chains
  const chainMap: Record<string, { to: string[]; type: string }> = {};
