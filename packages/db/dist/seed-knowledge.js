...
import { knowledgeAiAlternatives, knowledgeProficiencyCatalog, } from "./schema.js";
...
async function main() {
    ...
    const effort_hours = Object.entries(effortData.components).map(([id, comp]) => ({
        component_id: id,
        component_name: id.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
        base_hours: comp.base_hours,
        unit: comp.unit,
        includes: comp.includes,
        role_breakdown: comp.role_breakdown,
        phase_id: Object.entries(effortData.phase_mapping)
            .find(([, components]) => components.includes(id))?.[0] ?? "",
    }));
    // Build multipliers array
    const multipliers = multiplierData.multipliers.map((m) => ({
        multiplier_id: m.id,
    ...
    const gotcha_patterns = gotchaData.patterns.map((p) => ({
        pattern_id: p.id,
    ...
    const dependency_chains = chainData.dependencies.map((d) => ({
        chain_id: d.id,
    ...
    const phase_mappings = Object.entries(effortData.phase_mapping).map(([phaseId, componentIds], idx) => ({
        phase_id: phaseId,
        phase_name: phaseId.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
        phase_order: idx + 1,
        component_ids: componentIds,
    }));
    // Build roles array
    const roles = Object.entries(effortData.roles).map(([roleId, role]) => ({
        role_id: roleId,
    ...
    for (const [catId, cat] of Object.entries(profData.categories)) {
        await db
    ...
    await saveMigrationPath(db, {
        ...
        path_gotcha_patterns: gotchaData.patterns.map((p) => p.id),
