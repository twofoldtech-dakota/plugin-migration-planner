...
export async function saveMigrationPath(db, input) {
...
export async function getMigrationPath(db, input) {
    if (input.id) {
        const rows = await db
            .select()
            .from(migrationPaths)
    ...
    if (input.source_pack_id && input.target_pack_id) {
        const rows = await db
            .select()
            .from(migrationPaths)
            .where(and(eq(migrationPaths.source_pack_id, input.source_pack_id), eq(migrationPaths.target_pack_id, input.target_pack_id)))
            .limit(1);
        return rows[0] ?? null;
    }
    return null;
}
export async function listMigrationPaths(db, filters) {
    const conditions = [];
...
//# sourceMappingURL=migration-paths.js.map