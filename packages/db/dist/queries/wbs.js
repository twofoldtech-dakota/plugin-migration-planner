...
// ── List versions ────────────────────────────────────────────
export async function listWBSVersions(db, assessmentId) {
    const rows = await db
        .select({
...
// ── Save snapshot ────────────────────────────────────────────
export async function saveWBSSnapshot(db, input) {
...
// ── Get snapshot ─────────────────────────────────────────────
export async function getWBSSnapshot(db, assessmentId, version) {
    let snapshot;
    if (version) {
        const rows = await db
            .select()
            .from(wbsSnapshots)
            .where(and(eq(wbsSnapshots.assessment_id, assessmentId), eq(wbsSnapshots.version, version)))
            .limit(1);
        snapshot = rows[0];
    }
    else {
...
// ── Update work item ─────────────────────────────────────────
export async function updateWorkItem(db, itemId, updates) {
    const now = new Date().toISOString();
    await db
        .update(workItems)
        .set(updates)
...
// ── Create work item ─────────────────────────────────────────
export async function createWorkItem(db, snapshotId, item) {
    const now = new Date().toISOString();
    // Get max sort_order
...
// ── Delete work item ─────────────────────────────────────────
export async function deleteWorkItem(db, itemId) {
    const now = new Date().toISOString();
...
// ── Reorder work items ───────────────────────────────────────
export async function reorderWorkItems(db, snapshotId, orderedIds) {
    const now = new Date().toISOString();
    await db.transaction(async (tx) => {
        for (let i = 0; i < orderedIds.length; i++) {
            await tx
                .update(workItems)
                .set({ sort_order: i })
                .where(and(eq(workItems.id, orderedIds[i]), eq(workItems.snapshot_id, snapshotId)));
        }
    });
    await db
        .update(wbsSnapshots)
