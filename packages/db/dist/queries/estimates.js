...
export async function listEstimateVersions(db, assessmentId) {
    const rows = await db
        .select({
...
export async function saveEstimate(db, input) {
...
export async function getEstimate(db, assessmentId, version) {
    let snapshot;
    if (version) {
        const rows = await db
            .select()
            .from(estimateSnapshots)
            .where(and(eq(estimateSnapshots.assessment_id, assessmentId), eq(estimateSnapshots.version, version)))
            .limit(1);
        snapshot = rows[0];
    }
    else {
