...
import { getHeuristicsForPacks } from "./knowledge-heuristics.js";
...
// ── 1a. resolvePackIds ─────────────────────────────────────────
export async function resolvePackIds(db, assessmentId) {
...
export async function composeDiscoveryTree(db, assessmentId) {
...
export async function composeHeuristics(db, assessmentId, type) {
    const resolved = await resolvePackIds(db, assessmentId);
    const emptyResult = {
        effort_hours: [],
        multipliers: [],
    ...
    const sortedPackIds = [...resolved.packIds].sort((a, b) => (resolved.packPriority[a] ?? 99) - (resolved.packPriority[b] ?? 99));
    // Flatten and merge
    const effortHours = [];
    const multipliersSeen = new Map();
    const multipliers = [];
    const gotchaSeen = new Map();
    const gotchaPatterns = [];
    const chainsSeen = new Map();
