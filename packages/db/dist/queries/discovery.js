...
export async function saveDiscovery(db, input) {
...
export async function getDiscovery(db, assessmentId, dimension) {
    if (dimension) {
        const dims = await db
            .select()
            .from(discoveryDimensions)
            .where(and(eq(discoveryDimensions.assessment_id, assessmentId), eq(discoveryDimensions.dimension, dimension)))
            .limit(1);
        if (dims.length === 0)
            return null;
        const dim = dims[0];
        const rows = await db
            ...
            .where(and(eq(discoveryAnswers.assessment_id, assessmentId), eq(discoveryAnswers.dimension, dimension)));
        const answers = {};
