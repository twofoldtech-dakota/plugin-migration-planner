...
function loadMarkdown(relativePath) {
    const fullPath = resolve(KNOWLEDGE_DIR, relativePath);
...
async function main() {
    ...
    const scResult = await saveKnowledgePack(db, {
        ...
        description: "Sitecore Experience Platform (XP) — on-premises/IaaS digital experience platform with content management, personalization, marketing automation, and analytics.",
        direction: "source",
        latest_version: "10.4",
    ...
    const awsResult = await saveKnowledgePack(db, {
        ...
        description: "AWS cloud infrastructure — compute (EC2), databases (RDS), storage (S3), networking (VPC), and supporting services.",
        direction: "source",
    ...
    const azureResult = await saveKnowledgePack(db, {
        ...
        category: "infrastructure",
        subcategory: "hyperscaler",
        description: "Azure cloud infrastructure — compute (VMs/App Service), databases (SQL MI/SQL DB), storage (Blob), networking (VNet), and supporting services.",
        direction: "target",
        latest_version: "current",
        supported_versions: ["current"],
        eol_versions: {},
    ...
    const allInfraDims = monolithicTree.dimensions;
    ...
    const awsDims = allInfraDims.filter((d) => AWS_DIM_IDS.includes(d.id));
    const scInfraDims = allInfraDims.filter((d) => SC_INFRA_DIM_IDS.includes(d.id));
    ...
    const scInfraDimsNumbered = scInfraDims.map((d, i) => ({ ...d, order: 7 + i }));
    const scAllDims = [...scPlatformDims, ...scInfraDimsNumbered];
    const scTreeResult = await saveDiscoveryTree(db, {
        pack_id: "sitecore-xp",
        dimensions: scAllDims,
    });
    console.log(`  sitecore-xp discovery tree saved: ${scAllDims.length} dimensions ` +
    ...
    const allEffortHours = Object.entries(baseEffort.components).map(([componentId, data]) => ({
    ...
    const allMultiplierRows = multipliers.multipliers.map((m) => ({
        multiplier_id: m.id,
    ...
    const allGotchaRows = gotchas.patterns.map((p) => ({
        pattern_id: p.id,
    ...
    const allChainRows = deps.dependencies.map((d) => ({
        chain_id: d.id,
    ...
    const roles = Object.entries(baseEffort.roles).map(([roleId, data]) => ({
        role_id: roleId,
    ...
    const awsMultipliers = allMultiplierRows.filter((m) => AWS_MULTIPLIER_IDS.has(m.multiplier_id));
    const scMultipliers = allMultiplierRows.filter((m) => !AWS_MULTIPLIER_IDS.has(m.multiplier_id));
    const awsGotchas = allGotchaRows.filter((g) => AWS_GOTCHA_IDS.has(g.pattern_id));
    const scGotchas = allGotchaRows.filter((g) => !AWS_GOTCHA_IDS.has(g.pattern_id));
    const awsChains = allChainRows.filter((c) => AWS_CHAIN_IDS.has(c.chain_id));
    const scChains = allChainRows.filter((c) => !AWS_CHAIN_IDS.has(c.chain_id));
    // Phase mappings per pack
    const awsPhases = Object.entries(AWS_PHASE_MAPPING).map(([phaseId, componentIds], index) => ({
        phase_id: phaseId,
        phase_name: phaseId.replace(/^phase_\d+b?_/, "").replace(/_/g, " "),
        phase_order: index + 1,
        component_ids: componentIds,
    }));
    const scPhases = Object.entries(SC_PHASE_MAPPING).map(([phaseId, componentIds], index) => ({
        phase_id: phaseId,
        phase_name: phaseId.replace(/^phase_\d+b?_/, "").replace(/_/g, " "),
        phase_order: index + 1,
        component_ids: componentIds,
    ...
    console.log(`  aws heuristics saved: ${awsEffort.length} components, ${awsMultipliers.length} multipliers, ` +
    ...
    console.log(`  sitecore-xp heuristics saved: ${scEffort.length} components, ${scMultipliers.length} multipliers, ` +
    ...
    console.log(`  Totals: ${totalComponents} components (11+16), ${totalMultipliers} multipliers (16+25), ` +
        ...
        path_gotcha_patterns: [
            {
                ...
                risk_level: "medium",
                hours_impact: 12,
                description: "S3 SDK calls in custom Sitecore handlers/providers must be replaced with Azure Blob Storage SDK",
                mitigation: "Audit all code for AWS SDK references. Replace S3Client with BlobServiceClient. Update SAS token patterns.",
            ...
            },
            {
                ...
                risk_level: "high",
                hours_impact: 16,
                description: "AWS IAM role-based auth must be completely re-architected for Azure RBAC + Managed Identity",
                mitigation: "Map all IAM roles to Azure RBAC role assignments. Replace EC2 instance profiles with Managed Identity. Update all service-to-service auth.",
            ...
            },
            {
                ...
                risk_level: "low",
                hours_impact: 4,
                description: "RDS SQL Server connection strings use different format than Azure SQL MI. All Sitecore connection strings must be updated.",
                mitigation: "Update all ConnectionStrings.config entries. Replace RDS endpoint with Azure SQL MI endpoint. Update auth from SQL auth to Managed Identity if applicable.",
        ...
        path_effort_adjustments: [
            {
                ...
                adjustment_hours: 8,
                reason: "AWS VPC → Azure VNet translation requires CIDR replanning, NSG rule translation from security groups, and different subnet model",
            },
            {
                component_id: "managed_identity_keyvault",
                adjustment_hours: 8,
                reason: "IAM-to-RBAC migration adds overhead: mapping all IAM policies to Azure roles, replacing instance profiles with Managed Identity",
            },
        ],
...
- **Recommendation**: VPN for most cases; ExpressRoute if >500GB data or strict latency requirements`,
    ...
    console.log(`AWS Heuristics:   ${awsEffort.length} components, ${awsMultipliers.length} multipliers, ` +
        `${awsGotchas.length} gotchas, ${awsChains.length} chains`);
    console.log(`SC Heuristics:    ${scEffort.length} components, ${scMultipliers.length} multipliers, ` +
...
function findPhaseForComponent(componentId, phaseMapping) {
...
//# sourceMappingURL=seed-sitecore-knowledge.js.map